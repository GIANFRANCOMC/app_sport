<template>
    ...
    <section class="br-pos">
                    ...
                    <article
                        ...
                        <div class="br-pos-product__body">
                            ...
                            <span class="br-pos-product__info">
                                ...
                                <small>{{ item.brand?.name || item.description || 'Producto disponible' }}</small>
                                <span v-if="item.internal_code || item.barcode" class="br-pos-product__codes">
                                    <span v-if="item.internal_code">Cód. interno {{ item.internal_code }}</span>
                                    <span v-if="item.barcode">Cód. barras {{ item.barcode }}</span>
                                </span>
                            </span>
                        </div>
        ...
        <aside class="br-pos-ticket" aria-label="Resumen de venta">
            ...
            <section v-if="hasOpenCashSessions" class="br-pos-payments" aria-label="Métodos de pago">
                <div class="br-pos-payments__header">
                ...
                <div class="br-pos-payments-summary">
                    <div v-for="payment in paymentSummary" :key="payment.uid">
                        <span>{{ payment.label }}</span>
                        <strong>S/ {{ separatorNumber(payment.amount) }}</strong>
                    </div>
                </div>
                <div v-if="showPaymentEditor" class="br-pos-payments-editor">
                    <article v-for="(payment, index) in payments" :key="payment.uid" class="br-pos-payment">
                        <v-select
                    ...
                    <button type="button" class="br-pos-payments__add br-pos-payments__add-row" @click="addPayment">
            ...
            <section class="br-pos-ticket__items">
                ...
                <p v-if="!cart.length" class="br-pos-ticket__empty">Agrega productos para iniciar la venta.</p>
            </section>

            <button
                v-if="hasOpenCashSessions"
                type="button"
                class="br-btn br-btn-primary br-pos-ticket__pay"
                :disabled="saving || !canSubmit"
                @click="submitSale">
                <i class="fa-solid fa-cash-register" aria-hidden="true"></i>
                <span>{{ saving ? 'Generando venta' : 'Generar venta' }}</span>
            </button>
        </aside>
    </section>
</template>
